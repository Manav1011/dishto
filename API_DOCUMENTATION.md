...
## 🍽️ Restaurant Management Module
**Base URL:** `/api/protected/restaurant`

This module manages the core business hierarchy (Franchises, Outlets) and the Menu (Categories, Items). All endpoints generally require admin privileges for the respective scope (Franchise or Outlet).
...
### Menu Items & AI Features
*   **POST** `/{outlet_slug}/items`
    *   **Description:** Create a standard menu item with an image upload.
*   **POST** `/{outlet_slug}/items/no-image`
    *   **Description:** **(AI Powered)** Create a menu item *without* an optional image. The system automatically uses Google Gemini to generate a photorealistic image based on the item description.
...
*   **GET/PUT/DELETE** `/{outlet_slug}/transactions/{slug}`
    *   **Description:** Manage specific transaction records.
*   **POST** `/{outlet_slug}/orders`
    *   **Description:** Create a new customer order. This triggers stock deduction based on the recipes defined.
